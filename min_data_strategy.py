"""
几个思路先行落地：
    1.开盘5分钟最高价、最低价的算数平均值，开盘一个小时之内在此线上下的比例与收盘价的关系———已完成，效果一般
    2.5分钟k线策略，当个股的日线、60分钟线均处于强势/弱势时的5分钟策略
      （思路：通过技术指标的背离、大等级k线强势与短期形态的弱势确定进场点）
      (先从简单做起，均线金叉买进，再逐渐加指标）
      20210630，rb2110 11:10分，kd、rsi同时跌到20以下，成功的买入信号案例。
"""
import pickle
import pandas as pd
